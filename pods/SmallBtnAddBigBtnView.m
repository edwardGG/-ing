...
#import "SmallBtnAddBigBtnView.h"

@implementation SmallBtnAddBigBtnView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.farDistance = 60;
        self.endDistance = 30;
        self.mainView = [[BigBtn alloc] initWithFrame:CGRectMake(0, 0, 80, 80) Image:@"center"];
...
- (void)setMenuItems:(NSArray *)menuItems {
    if (_menuItems != menuItems) {
        ...
        for (int i = 0; i < count; i ++) {
            SmallBtn *item = [menuItems objectAtIndex:i];
            item.tag = 1000 + i;
...
- (void)expend:(BOOL)isExpend {
    _isExpend = isExpend;
    [self.menuItems enumerateObjectsUsingBlock:^(SmallBtn *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (self.scale) {
            if (isExpend) {
...
- (void)addRotateAndPostisionForItem:(SmallBtn *)item toShow:(BOOL)show {
    if (show) {
